...
RSpec.describe NewSuperCodebreaker2021::Game do
  ...
  context "#chose_com" do
    it "returns start symbol" do
      expect(game.chose_com("start")).to eq(:start)
    end

    it "returns rules symbol" do
      expect(game.chose_com("rules")).to eq(:rules)
    end

    it "returns stats symbol" do
      expect(game.chose_com("stats")).to eq(:stats)
    end

    it "returns exit symbol" do
      expect(game.chose_com("exit")).to eq(:exit)
    end

    it "returns false if bad user's input" do
      expect(game.chose_com("unknown_command")).to eq(false)
    end
  end
